...
def get_transforms(img_size: int= 224, rand_augment_magnitude: int=5):
    ...
    return {
        "train": transforms.Compose(
            [
                ...
                transforms.RandomPerspective(distortion_scale=0.2, p=0.5, interpolation=3),  # Random perspective
                transforms.GaussianBlur(3, sigma=(0.1, 2.0)),  # Random Gaussian blur
                # RandAugment
...
def get_data_loaders(
    batch_size: int = 32, valid_size: float = 0.2, 
    num_workers: int = -1, limit: int = -1, 
    img_size: int = 224, 
    ...
    data_transforms = get_transforms(img_size, rand_augment_magnitude)
...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    base_path = Path(get_data_location())
    class_names  = datasets.ImageFolder(base_path / "train").classes
